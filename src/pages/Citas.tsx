...
export const Citas: React.FC = () => {
  ...
  // 2. CORREGIDO: Escuchar la base de datos esperando a que cargue la sesión
  useEffect(() => {
    if (!usuario) return; // Frena la ejecución si la pestaña recién se abre y no hay sesión aún
    ...
    return () => desuscribir();
  }, [usuario]); // Se vuelve a activar en cuanto Firebase valida al usuario

  // BLINDAJE VISUAL: Mientras Firebase revisa la persistencia, congelamos la pantalla en negro
  if (cargando) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', backgroundColor: '#121214', color: '#fff', fontFamily: 'sans-serif' }}>
        <h2 style={{ color: '#e50914' }}>⚡ Sincronizando agenda de Ink & Needle...</h2>
      </div>
    );
  }
